...
const getByProgram = (req, res) => {
    ...
    res.status(400).json({message: `El programa con id ${programId} no existe`})
...
const getById = (req, res) => {
    ...
    if(!data){
            res.status(400).json({message: `El capitulo con id ${chapterId} no existe`})
    } 
...
const create = (req, res) => {
    ...
    if(programId){
        if(data.chapter_num){
            const response = chapterControllers.createChapter(data, programId, imgPath)
            res.status(201).json({message: `Chapter added succesfully with id: ${response.id}`})
        }
...
const edit = (req, res) => {
    ...
    if(response){
        res.status(200).json({message: 'Chapter edited succesfully', chapter: response})
    }
}

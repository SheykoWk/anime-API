...
const getChaptersByProgram = (programID, idprograms) => {
  const filterPrograms = programsDB.filter((programs) => programs.id === idprograms)
  const filterChapters = chaptersDB.filter((chapter) => chapter.program_id === programID);
  const data = Object.assign(filterPrograms, filterChapters)
...
const createChapter = (data, program_id, urlCover) => {
  const newChapter = {
    id: uuid.v4(),
